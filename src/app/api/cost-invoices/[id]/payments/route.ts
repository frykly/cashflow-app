  ...
  try {
    ...
    try {
      const row = await prisma.$transaction(async (tx) => {
        const created = await tx.costInvoicePayment.create({
          data: {
            costInvoiceId: id,
        ...
        await finalizeNewCostPaymentAllocations(tx, id, created.id, gNorm, explicit);
        return created;
      });
      await syncCostInvoiceStatus(id);
      const fresh = await prisma.costInvoicePayment.findUnique({
        where: { id: row.id },
        include: { projectAllocations: { include: { project: { select: { id: true, name: true } } } } },
      });
