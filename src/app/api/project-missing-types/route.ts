...
export async function GET() {
  const rows = await prisma.projectMissingTypeOption.findMany({
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const agg = await prisma.projectMissingTypeOption.aggregate({ _max: { sortOrder: true } });
    const sortOrder = (agg._max.sortOrder ?? 0) + 10;
    const row = await prisma.projectMissingTypeOption.create({
