...
export async function GET() {
  const rows = await prisma.projectLifecycleStatusOption.findMany({
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const agg = await prisma.projectLifecycleStatusOption.aggregate({ _max: { sortOrder: true } });
    const sortOrder = (agg._max.sortOrder ?? 0) + 10;
    const row = await prisma.projectLifecycleStatusOption.create({
