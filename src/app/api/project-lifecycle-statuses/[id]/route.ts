...
async function usageCounts(slug: string) {
  const projects = await prisma.project.count({ where: { lifecycleStatus: slug } });
  return { projects, total: projects };
}

export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  const { id } = await ctx.params;
  const row = await prisma.projectLifecycleStatusOption.findUnique({ where: { id } });
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await prisma.projectLifecycleStatusOption.update({
      where: { id },
      data: {
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.projectLifecycleStatusOption.delete({ where: { id } });
