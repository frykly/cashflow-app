/**
 * Audyt bazy SQLite przed migracją na Postgres (Krok 0).
 * Nie zmienia schema.prisma ani DATABASE_URL — używa bieżącego Prisma Client.
 *
 * Uruchom: npm run audit:sqlite
 * Wynik: reports/sqlite-audit-<timestamp>.{json,txt}
 */
...
async function tableCount(name: string): Promise<number> {
  const q = `SELECT COUNT(*) as c FROM "${name}"`;
  ...
  const checks: Array<{ label: string; sql: string }> = [
    {
      ...
      sql: `SELECT COUNT(*) as c FROM "ProjectTask" pt LEFT JOIN "Project" p ON p.id = pt.projectId WHERE p.id IS NULL`,
    },
    {
      label: "ProjectMissingItem bez Project",
      sql: `SELECT COUNT(*) as c FROM "ProjectMissingItem" m LEFT JOIN "Project" p ON p.id = m.projectId WHERE p.id IS NULL`,
    },
    {
      label: "ProjectMissingItem bez ProjectMissingTypeOption",
      sql: `SELECT COUNT(*) as c FROM "ProjectMissingItem" m LEFT JOIN "ProjectMissingTypeOption" t ON t.id = m.missingTypeId WHERE t.id IS NULL`,
    },
    {
      label: "ContractorAlias bez Contractor",
      sql: `SELECT COUNT(*) as c FROM "ContractorAlias" a LEFT JOIN "Contractor" c ON c.id = a.contractorId WHERE c.id IS NULL`,
    },
    {
      label: "BankTransaction bez BankImport",
      sql: `SELECT COUNT(*) as c FROM "BankTransaction" b LEFT JOIN "BankImport" i ON i.id = b.importId WHERE i.id IS NULL`,
    },
    {
      label: "IncomeInvoice.projectId orphan",
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoice" x LEFT JOIN "Project" p ON p.id = x.projectId WHERE x.projectId IS NOT NULL AND p.id IS NULL`,
    },
    {
      label: "IncomeInvoice.incomeCategoryId orphan",
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoice" x LEFT JOIN "IncomeCategory" c ON c.id = x.incomeCategoryId WHERE x.incomeCategoryId IS NOT NULL AND c.id IS NULL`,
    },
    {
      label: "CostInvoice.projectId orphan",
      sql: `SELECT COUNT(*) as c FROM "CostInvoice" x LEFT JOIN "Project" p ON p.id = x.projectId WHERE x.projectId IS NOT NULL AND p.id IS NULL`,
    },
    {
      label: "CostInvoice.expenseCategoryId orphan",
      sql: `SELECT COUNT(*) as c FROM "CostInvoice" x LEFT JOIN "ExpenseCategory" c ON c.id = x.expenseCategoryId WHERE x.expenseCategoryId IS NOT NULL AND c.id IS NULL`,
    },
    {
      label: "IncomeInvoicePlannedPayment orphan",
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoicePlannedPayment" r LEFT JOIN "IncomeInvoice" i ON i.id = r.incomeInvoiceId WHERE i.id IS NULL`,
    },
    {
      label: "IncomeInvoicePayment orphan",
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoicePayment" r LEFT JOIN "IncomeInvoice" i ON i.id = r.incomeInvoiceId WHERE i.id IS NULL`,
    },
    {
      label: "CostInvoicePayment orphan",
      sql: `SELECT COUNT(*) as c FROM "CostInvoicePayment" r LEFT JOIN "CostInvoice" i ON i.id = r.costInvoiceId WHERE i.id IS NULL`,
    },
    {
      label: "CostInvoiceProjectAllocation orphan",
      sql: `SELECT COUNT(*) as c FROM "CostInvoiceProjectAllocation" a LEFT JOIN "CostInvoice" i ON i.id = a.costInvoiceId LEFT JOIN "Project" p ON p.id = a.projectId WHERE i.id IS NULL OR p.id IS NULL`,
    },
    {
      label: "IncomeInvoiceProjectAllocation orphan",
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoiceProjectAllocation" a LEFT JOIN "IncomeInvoice" i ON i.id = a.incomeInvoiceId LEFT JOIN "Project" p ON p.id = a.projectId WHERE i.id IS NULL OR p.id IS NULL`,
    },
    {
      label: "PlannedEventProjectAllocation orphan",
      sql: `SELECT COUNT(*) as c FROM "PlannedEventProjectAllocation" a LEFT JOIN "PlannedFinancialEvent" e ON e.id = a.plannedFinancialEventId LEFT JOIN "Project" p ON p.id = a.projectId WHERE e.id IS NULL OR p.id IS NULL`,
    },
    {
      label: "OtherIncome.projectId orphan",
      sql: `SELECT COUNT(*) as c FROM "OtherIncome" o LEFT JOIN "Project" p ON p.id = o.projectId WHERE o.projectId IS NOT NULL AND p.id IS NULL`,
    },
    {
      label: "KsefDocument.syncSessionId orphan",
      sql: `SELECT COUNT(*) as c FROM "KsefDocument" d LEFT JOIN "KsefSyncSession" s ON s.id = d.syncSessionId WHERE d.syncSessionId IS NOT NULL AND s.id IS NULL`,
    },
  ];
...
async function largeTextStats(): Promise<Record<string, number>> {
  ...
  const queries: Array<[string, string]> = [
    ["IncomeInvoice.notes.maxLen", `SELECT IFNULL(MAX(LENGTH("notes")), 0) as m FROM "IncomeInvoice"`],
    ["CostInvoice.notes.maxLen", `SELECT IFNULL(MAX(LENGTH("notes")), 0) as m FROM "CostInvoice"`],
    ["BankTransaction.description.maxLen", `SELECT IFNULL(MAX(LENGTH("description")), 0) as m FROM "BankTransaction"`],
    ["BankTransaction.dedupeInputText.maxLen", `SELECT IFNULL(MAX(LENGTH("dedupeInputText")), 0) as m FROM "BankTransaction"`],
    ["KsefDocument.rawPayload.maxLen", `SELECT IFNULL(MAX(LENGTH("rawPayload")), 0) as m FROM "KsefDocument"`],
    ["BankImport.skippedLinesJson.maxLen", `SELECT IFNULL(MAX(LENGTH("skippedLinesJson")), 0) as m FROM "BankImport"`],
  ];
  ...
  const checks: Array<{ label: string; sql: string }> = [
    {
      ...
      sql: `SELECT COUNT(*) as c FROM "IncomeInvoice" WHERE "invoiceNumber" IS NULL`,
    },
    {
      label: "CostInvoice.documentNumber NULL",
      sql: `SELECT COUNT(*) as c FROM "CostInvoice" WHERE "documentNumber" IS NULL`,
    },
    {
      label: "User.email / passwordHash NULL",
      sql: `SELECT COUNT(*) as c FROM "User" WHERE "email" IS NULL OR "passwordHash" IS NULL`,
    },
  ];
  for (const { label, sql } of checks) {
    const rows = await prisma.$queryRawUnsafe<{ c: bigint | number }[]>(sql);
...
async function main() {
  ...
  const base = resolve(reportsDir, `sqlite-audit-${stamp}`);
  ...
  function fmtSqliteDate(v: string | null): string | null {
  ...
  async function rng(model: string, field: string) {
    try {
      const rows = await prisma.$queryRawUnsafe<{ mn: string | null; mx: string | null }[]>(
        `SELECT MIN("${field}") as mn, MAX("${field}") as mx FROM "${model}"`,
      );
      dateRanges[`${model}.${field}`] = {
        min: fmtSqliteDate(rows[0]?.mn ?? null),
        max: fmtSqliteDate(rows[0]?.mx ?? null),
      };
    } catch {
  ...
  const issues: string[] = [];
  if (userCount === 0) issues.push("Brak użytkowników (User: 0) — po imporcie na Postgres trzeba npm run create-admin.");
  ...
  const txt = [
    `SQLite audit — ${summary.generatedAt}`,
    "",
